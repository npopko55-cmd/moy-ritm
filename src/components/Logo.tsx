 ...
 *
 * Идентификатор градиента — свой у каждого знака, и это не украшение.
 * На экране паузы знаков на странице два: один в плеере, который спрятан
 * под экраном паузы, второй в её шапке. Одинаковый id ломал второй: ссылка
 * `url(#star-grad)` находит ПЕРВЫЙ элемент в документе, а он лежит внутри
 * `display: none`, где браузер градиент не строит, — заливки не получалось
 * ни у кого, и от логотипа оставалась одна надпись.
 */
export function StarMark({ size = 40 }: { size?: number }) {
  // useId выдаёт «:r3:» — двоеточия из идентификатора убираем, чтобы он
  // одинаково читался и как id, и внутри url(...).
  const gradId = `star-grad-${useId().replace(/[^a-zA-Z0-9_-]/g, '')}`
  return (
