 ...
 * Тарифы подписки.
 *
 * Форма записи повторяет ответ бэкенда на GET /api/v1/tariffs. Страница
 * тарифов берёт цены с сервера; отсюда их берут демо-режим (src/api/demo.ts)
 * и блок разблокировки — как запасной вариант, пока ответ не пришёл.
 *
