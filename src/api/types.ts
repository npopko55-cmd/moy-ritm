 ...
 * id, название, описание и признак `locked`, а списки движений и треков
 * пустые — ссылок на видео и музыку в закрытый поток не попадает.
 *
...
export class ApiError extends Error {
  ...
  /**
   * Сбой временный: сети нет, сервер перезапускается (5xx), просит подождать
   * (429) или не дождался запроса (408). Это не отказ по существу — из-за
   * такого ответа вход не сбрасываем, а пробуем ещё раз.
   */
  ...
  /** Ответ не пришёл вовремя — зависшая мобильная сеть. Для человека это та же «нет связи». */
